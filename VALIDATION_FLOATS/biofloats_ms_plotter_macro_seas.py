...
def argument():
    ...
    parser.add_argument(   '--nyears', '-y',
                                type = str,
                                default = None,
                                required = True,
                                help = "it helps at slicing the season FMA -MJJ script hardcoded")    
...
from profilerDAf_RSTbefore import TL
from profilerDAf_RSTbefore import *
import sys
...
if N_YRS==2:
   INVERNO_SETTMANE = np.append(np.arange(5,18) , np.arange(57,70))  
   ESTATE_SETTIMANE = np.append(np.arange(22,36) , np.arange(75,88))
if N_YRS==1:
   INVERNO_SETTMANE = (np.arange(5,18))
   ESTATE_SETTIMANE = (np.arange(22,36))

print('INVERNO E ESTATE DATE A MANO NELLO SCRIPT')

for ivar, var in enumerate(VARLIST):
    RMSE_w = np.zeros((nSub,nLayers),np.float32)
    RMSE_s = np.zeros((nSub,nLayers),np.float32)
    for isub, sub in enumerate(SUBLIST):
        tmp_w_RMSE = DATAfile.rmse[ivar,[INVERNO_SETTMANE],isub,:]
        tmp_w_RMSE = np.nanmean(tmp_w_RMSE, axis=1)      
        RMSE_w[isub,] = tmp_w_RMSE

        tmp_s_RMSE = DATAfile.rmse[ivar,[ESTATE_SETTIMANE],isub,:]
        tmp_s_RMSE = np.nanmean(tmp_s_RMSE, axis=1)
        RMSE_s[isub,] = tmp_s_RMSE
    print(RMSE_w)        
    writetable(OUT_TABLEDIR + 'Winter_FMA'  + '_' + var + '_RMSE.txt',RMSE_w, row_names, column_names)
    writetable(OUT_TABLEDIR + 'Summer_JJA'  + '_' + var + '_RMSE.txt',RMSE_s, row_names, column_names)


def single_plot_LF(longvar, var, sub, layer, timeinterval ):
...
for ivar, var in enumerate(VARLIST):
    for isub, sub in enumerate(SUBLIST):
        for ilayer, layer in enumerate(LAYERLIST):
            ...
            fig,ax,ax2  = single_plot_LF(VARLONGNAMES[ivar],var,sub.name,layer.string(), ti_restrict)
