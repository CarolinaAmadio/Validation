...
for ivar_m, var_mod in enumerate(VARLIST):
    ...
    for wmo in wmo_list:
        ...
        if (var_mod == 'N3n'):
            quadmesh = ax3.pcolormesh(xs, ys, plotmat_m,shading='nearest',vmin=0.00,vmax=8) #,cmap="jet")# default is 'flat'
            quadmesh = ax4.pcolormesh(xs, ys, plotmat_model,shading='nearest',vmin=0.00,vmax=8) #,cmap="jet")# default is 'flat'
        if (var_mod == 'P_c'):
