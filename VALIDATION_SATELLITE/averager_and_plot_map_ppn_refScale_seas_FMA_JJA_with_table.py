...
#
if OUTDIR.capitalize().startswith('I'):
   RUN_='DAfl'
   prex= 'i'
elif OUTDIR.capitalize().startswith('H'):
   RUN_='HIND'
   prex='h'
elif OUTDIR.capitalize().startswith('S'):
   RUN_='DAnn'
   prex='s'
else:
   sys.exit('che run???') 
...
if os.path.isfile(ncfile):
   ...
   for il, layer in enumerate(PLOT.layerlist):
       ...
       outfile    = OUTDIR +prex+'_' + iseas+ "_Map_" + var + "_" + req_label + "_Ave" + layer.longname() + z_mask_string  + "_mgm2d.png"
...
# X ppn:
    ...
    #pl.savefig(prex +'_' +outfile, dpi=300)
